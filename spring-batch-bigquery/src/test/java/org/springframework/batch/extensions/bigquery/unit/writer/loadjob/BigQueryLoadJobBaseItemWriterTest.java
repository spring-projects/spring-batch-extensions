...
package org.springframework.batch.extensions.bigquery.unit.writer.loadjob;
...
class BigQueryLoadJobBaseItemWriterTest extends AbstractBigQueryTest {
	...
	void testSetDatasetInfo() throws IllegalAccessException, NoSuchFieldException {
		TestWriter writer = new TestWriter();
		MethodHandles.Lookup handle = MethodHandles.privateLookupIn(BigQueryLoadJobBaseItemWriter.class,
				MethodHandles.lookup());
		DatasetInfo expected = DatasetInfo.of(TABLE_ID.getDataset());

		writer.setDatasetInfo(expected);

		DatasetInfo actual = (DatasetInfo) handle
			.findVarHandle(BigQueryLoadJobBaseItemWriter.class, "datasetInfo", DatasetInfo.class)
			.get(writer);
	...
	void testSetWriteChannelConfig() throws IllegalAccessException, NoSuchFieldException {
		...
		WriteChannelConfiguration actual = (WriteChannelConfiguration) handle
			.findVarHandle(BigQueryLoadJobBaseItemWriter.class, "writeChannelConfig", WriteChannelConfiguration.class)
			.get(writer);
	...
	void testWrite_Exception() throws Exception {
		...
		boolean writeFailed = (Boolean) handle
			.findVarHandle(BigQueryLoadJobBaseItemWriter.class, "writeFailed", boolean.class)
			.get(writer);
	...
	void testBaseAfterPropertiesSet_Dataset() throws IllegalAccessException, NoSuchFieldException {
		...
		TestWriter writer = new TestWriter();
		writer.setBigQuery(bigQuery);
		writer.setWriteChannelConfig(
				WriteChannelConfiguration.newBuilder(TABLE_ID).setFormatOptions(FormatOptions.json()).build());
	...
	void testTableHasDefinedSchema() {
		...
		// With schema
		Mockito.when(table.getDefinition())
			.thenReturn(definitionBuilder.setSchema(Schema.of(Field.of(TestConstants.AGE, StandardSQLTypeName.STRING)))
				.build());
